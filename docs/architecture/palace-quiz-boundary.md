...
- Bidirectional UI:
  - Map/review → `NodeBoundQuizDialog` overlay (owner labels 本宫 / 来自·他宫); each card shows up to two corner badges for the subtree union of bound questions (including cross-owner edges): **客观** (every type except `short_answer`, green) and **主观** (`short_answer`, sky). A side with zero questions is omitted. Counts are the full totals and do not shrink when a question is completed; session progress stays in the quiz progress UI. If that objective or subjective set contains a marked question, that badge uses a rose fill. Clicking a badge opens only that side's questions. Subject bookshelf palace cards reuse that same `NodeCountBadgeCluster` outside the card. The shelf count is the distinct non-deleted questions bound to the palace (the root-node total when every edge still points at a live node). Clicking a shelf badge opens that palace’s quiz page. Session 已做 is shared with Freestyle toolbar overlay and Palace Quiz practice; reload / 再来一轮 / scoring that palace’s review units clears it. Prev/next still keeps resolved states so the learner can review prior answers. Quiz practice uses 标记 / 取消标记 instead of 忘记/困难/记得/轻松. Marking writes `marked` and does not schedule review or auto-advance. A last stored 忘记 or 困难 rating (unpassed, with a due date) migrates to marked. Marked question numbers on the index rail use a rose fill.
