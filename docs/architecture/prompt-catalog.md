...
2. Template defaults, persisted overrides, placeholders, and rendering remain settings-owned.
...
9. All interactive AI runs use `prompt_options` composition; full-template override APIs are retired.
10. Browser storage may retain model choice and unfinished run instructions, but never replaces the server scene default.
...
Prompt composition uses scene defaults and modular blocks only. The retired full-template override and evaluation APIs are not part of the current catalog.
...
- Scene defaults expose user-facing `label` / `category` for selection; AI 分卡 uses the unified scene key `ai_split`.
