...
8b. Freestyle passes `autoRevealNonDueCards={false}` so unit flip is classic **hidden → 待回忆 → content** for every node. Unit may have many flip cards while formal FSRS freeze only includes **due** nodes in that unit (header `N 节点 · M 到期` is intentional).
