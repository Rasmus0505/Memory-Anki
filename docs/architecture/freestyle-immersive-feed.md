...
4b. Optional `include_calendar_today_due` (default false): freestyle mind-map due set also includes formal-eligible nodes whose `due_at` falls on the **local calendar day** even if the clock time has not arrived. Clock-due and available reinforcement always count. Does not change formal overview / Insights due counts. Scoped freestyle freezes may rate those calendar-today nodes when they appear on the card.
...
8b. Freestyle and formal palace review both use `autoRevealNonDueCards={false}` so flip is classic **hidden → 待回忆 → content** for every node (including non-due). Unit/formal freeze still only **rates** due nodes; non-due are **soft-dimmed** on the canvas. Header `N 节点 · M 到期` remains intentional for freestyle units.
