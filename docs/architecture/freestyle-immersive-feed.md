...
8. The freestyle mind-map card UI **clips** `editor_doc` to the queue unit via `clipEditorStateToBranchUnit`: a **single-child spine from the real palace root** down to the unit branch (original node titles, no `A / B / C` path-string root), then the complete unit subtree. Sibling branches are stripped. Context path remains header-only orientation. Flip starts root-only and advances one level at a time like the original mind map. Flip/due/rating scope still uses `ratable_node_uids` / unit `due_node_uids`.
...
8c. Freestyle mind-map **flip/reveal snapshots** are cached per card id (`branchRevealSnapshotCache`). Swiping away unmounts cards outside the mount window; swiping back restores the previous reveal map so the unit does **not** collapse to root-only. Silent queue rebuilds with the same card id must not hard-reload a warm unit either.
...
## Split marks (permanent + temporary)

Mark points are freestyle **split anchors**. Temporary and permanent use **the same topology**; only lifecycle differs.

- **Shared topology**: `split_anchors = permanent_mark_uids ∪ temporary_root_uids` (all active temporary marks, including nested L2/L3 — not outermost-only). **Water-pour model**: pour from the root downward; unmarked nodes never cut the stream. Hitting a mark starts a unit that owns the mark plus every descendant until the next deeper mark (deeper mark subtrees are separate units). Unmarked sibling branches under the same mark stay together (e.g. L1 hub + four unmarked children + one L2 → 2 units, not five). `node_limit` best-fit applies only in fully unmarked residual regions after mark regions are claimed. Unmarked ancestors above a mark fold into that mark's unit. Nested levels: level = 1 + count of marked ancestors.
- **Permanent split marks** live on palace editor node data (`permanentSplitMark: true`). They only affect freestyle `split_branch_units` and are never cleared by rating. Formal palace review does not read them.
- **Temporary marks** are freestyle-only overrides stored in `freestyle_temporary_marks`. Confirming marks can permanently unify FSRS progress across the union of all marked uids' subtrees (average of nodes that already have FSRS; write back to the whole group). Nested marks are all saved. Marks clear after Good/Easy settlement of their unit and must persist until then.
- **Only difference is time** (lifecycle), not split geometry. `selection_reason`: temporary wins over permanent when a unit root is in both sets.
...
- mix_mode is the primary control for how palace-side cards (mindmap + anki) and quiz cards interleave: mindmap_only | quiz_only | sequential_map_quiz | sequential_quiz_map | ratio | random.
- mix_ratio (mindmap : quiz, 1–10) applies when mix_mode=ratio (default 2:1).
- bound_quiz_placement: follow_unit (default, bound quizzes stick after their branch), into_mix, or quiz_stream.
