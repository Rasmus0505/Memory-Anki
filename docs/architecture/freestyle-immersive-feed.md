...
- **Permanent split marks** live on palace editor node data (`permanentSplitMark: true`). Levels are auto-derived from marked ancestors (L1/L2/…). They only affect freestyle `split_branch_units` (force cut at anchors). Formal palace review does not read them.
