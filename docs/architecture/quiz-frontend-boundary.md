...
The Palace memory lookup dialog composes Palace data, review feedback, and the mind-map editor, so it belongs to `widgets/palace-memory-lookup`. Palace Quiz and Freestyle consume the widget without importing one another. When a caller passes the current question’s bound `focusNodeUid`, the dialog keeps the full palace `editor_doc` and visually centers that node via `focusRequestNodeUid` (`resolveMemoryLookupFocusNodeUid`). It must not re-root or clip the tree to the bound node. Node-bound question badges use `widgets/node-bound-quiz`. Freestyle toolbar 做题 uses `widgets/freestyle-scope-quiz`: same answering chrome, question list from the saved feed palace range plus `overlay_question_range` (`due` / `all`). Session 已做 (answered drafts) is shared across node-bound badges, the overlay, and Palace Quiz practice for the SPA lifetime; a reload, 「再来一轮」, or scoring every review unit of a palace clears that 已做. After an answer, all three surfaces show 忘记/困难/记得/轻松 and write question-owned first-learning due dates. The first rating auto-advances; changing a rating does not. Due questions are amber on the index rail. Overlay membership and parked progress still live on `overlay_quiz`. A palace’s overlay is dropped after that palace’s review units are scored. It must not switch `training_mode` or open `QuizLauncher`. New cross-scene quiz primitives should be added to the Quiz entity; new multi-feature visual composition should be added to a widget or page.
