...
knowledge.application -> reviews.api (read-only unit projections for chapter palace cards)
```
...
Chapter palace list cards may call the Reviews public facade for permanent-mark count, unit count, due-unit count, and next review date. Knowledge must not import private review application modules or mutate review-unit state.
