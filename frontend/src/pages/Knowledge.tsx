...
import { BookOpen, FolderTree, Plus, Save, Trash2 } from 'lucide-react'
...
export default function Knowledge() {
  ...
  const {
    ...
    fetcher: api.getSubjectEditor,
    saver: api.saveSubjectEditor,
  ...
  useEffect(() => {
    void api.getSubjects().then((items) => {
      setSubjects(items)
  ...
  const handleCreateSubject = async () => {
    ...
    if (!name) return
    const subject = await api.createSubject({ name, color: '#6366f1' })
    setNewSubjectName('')
  ...
  const handleSaveSubject = async () => {
    ...
    if (!nextName) return
    await api.updateSubject(activeSubject.id, { name: nextName, color: subjectColor })
  ...
  const handleDeleteSubject = async () => {
    if (!activeSubject) return
    await api.deleteSubject(activeSubject.id)
    setSelectedNodes([])
    setChapterDetail(null)
