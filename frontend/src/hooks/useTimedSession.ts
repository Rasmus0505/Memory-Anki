...
export interface TimedSessionController {
  ...
  complete: (method: SessionCompletionMethod, meta?: Record<string, boolean | number | string | null>) => TimeSessionRecord | null
  reset: () => void
}
...
export function useTimedSession({
  ...
  const complete = React.useCallback((method: SessionCompletionMethod, meta?: Record<string, boolean | number | string | null>) => {
    ...
    setStatus('completed')
    setGlowState('idle')
    pushEvent(method === 'auto_complete' ? 'auto_complete' : method === 'manual_complete' ? 'manual_complete' : 'complete', meta)
