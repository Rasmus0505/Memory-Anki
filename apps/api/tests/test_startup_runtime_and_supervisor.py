...
class StartupModeTests(unittest.TestCase):
    ...
    def test_runtime_health_route_returns_startup_mode(self):
        ...
        with (
            patch.dict(os.environ, {"MEMORY_ANKI_STARTUP_MODE": "healthcheck"}, clear=False),
            patch.object(
                settings_router,
    ...
    def test_startup_warmup_swallows_and_logs_errors(self):
        ...
        with (
            patch.object(startup_warmup, "run_startup_warmup", side_effect=RuntimeError("boom")),
            patch.object(
    ...
    def test_startup_warmup_reads_node_due_index_without_stage_health_check(self):
        ...
        with patch.object(startup_warmup, "get_session", return_value=session):
    ...
    def test_lifespan_does_not_start_warmup_in_healthcheck_mode(self):
        ...
        async def run_lifespan():
            with (
                patch.object(app_main, "resolve_startup_mode", return_value="healthcheck"),
                patch.object(
                    app_main,
