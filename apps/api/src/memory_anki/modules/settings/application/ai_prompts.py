...
核心目标：
- 优先还原原文层级，不要把教材内容总结成你自己的话。
- 允许主动推断父子关系，不要只依赖破折号、冒号、编号这些显式格式。
- 如果一句话本身表达的是“主题 -> 结论”“栏目 -> 分项”“定义 -> 内容”，默认拆成父子结构。
...
{{semantic_split_rule}}
{{emphasis_rule}}
7. 子节点默认提取原文里的结论短语、分项短语、阶段短语，不要重复保留主语套话。
8. 例如：
   - “性质和目的——为主效力” 应拆成父节点“性质和目的” + 子节点“为主效力”
   - “教育内容：早期……；后期……” 应拆成父节点“教育内容” + 子节点“早期……”和“后期……”
   - “修道院的性质和目的是为主效力” 应拆成父节点“性质和目的” + 子节点“为主效力”
9. 默认要主动识别并拆开的句型包括：
   - X——Y
   - X：A；B
   - X是Y
   ...
   - X包括A、B
   - X分为A、B
10. 不要额外生成教材里没有的新总结语；如果原文只有短语，就输出短语。
11. 输出格式必须为：
{
  "title": "根节点标题",
  "children": [
...
核心目标：
- 优先抽出页面里本来就存在的层级关系，不要把教材内容总结成你自己的话。
- 允许主动推断父子关系，不要只盯着破折号、冒号、编号。
- 如果一句话本身表达的是“主题 -> 结论”“栏目 -> 分项”“定义 -> 内容”，默认拆成父子结构。
...
5. 父节点优先取主题词、栏目名、定义项；子节点优先取原文里的结论短语、分项短语、阶段短语。
6. 不要把“性质和目的——为主效力”“修道院的性质和目的是为主效力”这种内容放进同一个节点里，应该拆成父子节点。
7. 默认要主动识别并拆开的句型包括：
   - X——Y
   - X：A；B
   - X是Y
   ...
   - X包括A、B
   - X分为A、B
8. 不要额外生成教材里没有的新总结语；如果原文只有短语，就输出短语。
{{quote_rule}}
{{semantic_split_rule}}
...
PROMPT_DEFINITIONS: dict[str, PromptTemplateDefinition] = {
    "ai_prompt_import_image_mindmap": PromptTemplateDefinition(
        ...
        label="图片转脑图",
        description="单张图片识别脑图结构时发送给视觉模型的完整提示词。",
    ...
    "ai_prompt_import_batch_mindmap": PromptTemplateDefinition(
        ...
        label="多图合成脑图",
        description="结构图 + 正文图合成脑图时的完整提示词模板。",
...
def build_import_pdf_direct_prompt(
    *,
    ...
    prompt = render_prompt(
        ...
        {
            "quote_rule": (
                "9. 节点内容默认尽量使用原文，不要随意改写；仅在明显不适合脑图展示时做最小压缩，并优先保留原文短语。"
                if getattr(import_options, "quote_original_text_only", True)
                else "9. 允许把原文适度压缩成更适合脑图展示的短语，但必须忠实原意，不能捏造，也不要写教材外总结。"
            ),
            "semantic_split_rule": (
                "10. 遇到长段正文时，优先识别其中已经存在的分类、阶段、定义、目的、内容等关系，拆成父子或并列知识点，避免把整段塞进单个节点。"
                if getattr(import_options, "semantic_split_long_paragraphs", True)
                else "10. 不要为了美化结构主动改写长段正文，但遇到原文本身已经明显表达的分类、阶段、定义关系时仍应拆出层级。"
            ),
            "emphasis_rule": (
                "11. 如果页面里存在下划线或波浪线强调，尽量在 rich_text_html 和 emphasis_marks 中保留。"
                if getattr(import_options, "preserve_emphasis_marks", True)
                else "11. 无需额外保留下划线或波浪线强调，只要内容归位正确即可。"
            ),
            "ocr_grounding_block": ocr_grounding_block,
        },
        session=session,
