...
IMPORT_IMAGE_MINDMAP_PROMPT = """你是一个严格输出 JSON 的助手。

任务：把输入内容整理成思维导图 JSON。

强制要求：
1. 只输出 JSON，不要 markdown，不要解释。
2. 保留原文内容，不要总结、删减或编造。
3. 根据需要分为并列节点或变为父子结构，但保留原文内容。
4. 结构保真：
   - 若是单张文字卡/一段连续文字：输出 {"title":"图中全文","children":[]}
   - 若是多节点导图：才拆成树，节点文字与图中标签一致
5. 顶层格式：{"title":"根节点标题","children":[{"text":"节点文字","children":[]}]}
6. 每个节点必须有 text 与 children（叶子为 children: []）。根节点用 title，子节点用 text。
7. 知识重点（黄色底色）：若本次运行提供了“重点标记线索”，在对应节点可选输出
   emphasis_marks:[{"kind":"highlight","text":"原文子串"}]；
   节点 text 保持纯文本；emphasis_marks.text 必须是该节点 text 的子串；
   不要用 markdown 标记重点；未提供线索可不输出；产品侧渲染为黄色底色。
8. 禁止输出编辑器内部字段（data、uid、richText、theme、layout 等）。

格式整理：
1. 合理分卡：并列要点拆成并列 children；确有从属/分类关系时再做父子。
"""

# 兼容旧配置键；运行时多图/PDF 已改为「先全文识别再整理」，不再直出脑图。
IMPORT_DOCUMENT_MINDMAP_PROMPT = IMPORT_IMAGE_MINDMAP_PROMPT

IMPORT_OCR_MINDMAP_FORMAT_PROMPT = """你是一个严格输出 JSON 的助手。

任务：下面是已经识别出的上传页全部原文。请按目标范围删除多余内容（页眉页脚、广告、范围外章节等），整理成思维导图 JSON。

强制要求：
1. 只输出 JSON，不要 markdown，不要解释。
2. 严格保留目标范围内的原文内容，不要总结、删减或编造。
3. 根据需要分为并列节点或变为父子结构，但保留原文内容。
4. 结构保真：
   - 若是单张文字卡/一段连续文字：输出 {"title":"全文","children":[]}
   - 若是多节点结构：才拆成树，节点文字与原文一致
5. 顶层格式：{"title":"根节点标题","children":[{"text":"节点文字","children":[]}]}
6. 每个节点必须有 text 与 children（叶子为 children: []）。根节点用 title，子节点用 text。
7. 知识重点（黄色底色）：若本次运行提供了“重点标记线索”，在对应节点可选输出
   emphasis_marks:[{"kind":"highlight","text":"原文子串"}]；
   ...
   不要用 markdown 标记重点；未提供线索可不输出；产品侧渲染为黄色底色。
8. 禁止输出编辑器内部字段（data、uid、richText、theme、layout 等）。
9. 若提供了目标章节标题，以其为范围参考；遇到下一同级章节时停止。
...
IMPORT_IMAGE_TEXT_PROMPT = """你是一个严格输出纯文本的助手。

任务：识别上传页面中的全部文字内容。

要求：
1. 只输出纯文本，不要 markdown，不要 JSON，不要解释。
2. 尽量完整识别，不要总结，不要省略。
3. 保留段落、换行、列表顺序与层级。
4. 不要添加图中不存在的内容。
"""
