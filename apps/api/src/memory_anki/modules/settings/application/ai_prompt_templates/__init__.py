...
2. 保留原文要点，不要总结删减。
3. 顶层格式：{"title":"根节点标题","children":[{"text":"节点文字","children":[]}]}
4. 每个节点必须有 text 与 children（无子节点也要 children: []）。
"""
...
要求：
1. 只输出 JSON，不要 markdown，不要解释。
2. 严格保留目标范围内的原文要点，不要总结、删减或编造。
3. 按原文标题/编号/段落建立层级；并列要点拆成并列 children。
4. 顶层格式：{"title":"根节点标题","children":[{"text":"节点文字","children":[]}]}
5. 每个节点必须有 text 与 children（无子节点也要 children: []）。
6. 若提供了目标章节标题，以其为范围参考；遇到下一同级章节时停止。
