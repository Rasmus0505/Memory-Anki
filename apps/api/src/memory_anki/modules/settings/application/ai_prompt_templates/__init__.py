...
2. 页面中的精华提要、目录、表格或已有示意图只能辅助判断层级，不能代替正文内容。
3. 严格保留目标章节范围内的全部原文内容，不要总结、删减、改写或补充图片中不存在的内容；可以按原意拆成并列要点。
4. 遇到下一同级章节标题时立即停止，不要把下一章节及之后内容放入当前脑图。
5. 跨页时，下一页顶部属于当前章节的续文必须保留，直到下一同级章节真正开始。
6. 排除页眉、页脚、页码、广告、QQ群、版权水印和扫描水印。
...
9. 每个节点必须有非空 text 和数组 children；没有子节点也必须输出 children: []。
10. 多个并列要点必须拆成多个并列 children，不要塞进一个超长节点。
11. 输出前检查 JSON 括号、引号和数组均已完整闭合。
12. emphasis_marks 为可选字段：仅当用户提供重点标记线索时，将符合线索的原文片段写入 marks；text 保持纯文本，marks.text 必须是节点 text 的子串。产品侧渲染为黄色底色。
"""
...
2. 严格保留目标章节范围内的全部原文内容，不要总结、删减、改写或编造；允许按原意拆分并列要点。
3. 遇到下一同级章节标题时立即停止；保留下一页顶部属于当前章节的续文。
4. 排除页眉、页脚、页码、广告、QQ群、版权水印和扫描水印。
5. 若目标章节标题非空，以它作为识别边界和根标题参考；否则用 OCR 原文中的首个最高级标题确定根标题。
6. 只输出 JSON，不要输出 markdown，不要输出解释。
7. 顶层格式必须为 {"title":"根节点标题","children":[{"text":"节点文字","children":[]}]}。
8. 每个节点必须有非空 text 和数组 children；至少包含一个有效内容节点。
9. 输出前检查 JSON 完整闭合。
10. 若用户提供重点标记线索，可在节点上输出 emphasis_marks：[{"kind":"highlight","text":"原文子串"}]；text 保持纯文本。
