...
IMPORT_PDF_PAGE_CONTEXT_PROMPT = """附加约束：
1. 只处理本次给出的 PDF 选定页面，不要假设整本书的其他页面内容。
2. 用户给出的页码范围和自然语言提示只是帮助你聚焦本次识别范围，不能编造未出现的内容。
"""

IMPORT_PDF_STRUCTURE_TEMPLATE = """你是一个严格输出 JSON 的 PDF 脑图结构还原助手。
...
IMPORT_PDF_MERGE_TEMPLATE = """你是一个严格输出 JSON 的 PDF 脑图正文补充助手。
...
IMPORT_PDF_DIRECT_TEMPLATE = """你是一个严格输出 JSON 的 PDF 转脑图助手。
...
def build_palace_quiz_pdf_pairing_prompt(extra_prompt: str) -> str:
    ...
    return (
        "你是题目册-答案册配对助手。根据视觉模型初稿和资料角色说明，把候选整理成程序可识别的最终题库 JSON。\n"
        "视觉初稿可能是 question_candidates/answer_candidates；你必须把它们整理成最终 questions 数组，禁止原样返回候选字段。\n"
        "你必须只根据候选内容判断最终题型，不允许凭空补题或脱离候选改写。\n"
        "覆盖规则：逐条检查 question_candidates。每一道位于用户补充范围内、并且能在 answer_candidates 中找到答案依据的题，"
        "都必须输出为一条最终 questions；不要只挑真题、重点题或前几题。\n"
        "真题典例、例题、模拟练习、选择题、简答题、论述题只要属于所选范围且可配对，都必须保留；"
        "不要因为栏目叫“模拟练习”或题型是主观题就删除。\n"
        "规则：题目来源提供题干/选项，答案来源提供答案/解析；按栏目+题号优先配对，无法配对的题不要输出。\n"
        "同号但栏目不同的题不能合并，例如真题典例1和模拟练习1是两道题。\n"
        "如果用户补充要求引用的是某一章、某一节、某几页或某个页面范围，则该范围内的真题、例题、模拟练习等子栏目都视为属于该范围；"
        "除非用户明确要求排除某个栏目，否则不要因为子栏目标题不同就删题。\n"
        "范围判断不能只依赖候选里的 section 字段：必须同时查看 stem、options、source_snippet、analysis 的实际主题。"
        "如果 section 看似匹配但题干或片段明显属于相邻章节、上一页续题、下一节、其他人物/国家/时期/制度或其他主题，"
        "应视为 out_of_scope 跳过，不要把它硬归入用户要求的范围。\n"
        "如果候选显示是选择题，必须保留题目来源原始选项文字和 A/B/C/D 顺序，禁止重排、替换或按答案重写选项。\n"
        "选择题要输出 {\"question_type\":\"multiple_choice\",\"stem\":\"...\",\"options\":[{\"id\":\"A\",\"text\":\"...\"}],\"correct_option_id\":\"A\",\"analysis\":\"...\"}。\n"
        "根据答案/解析文字选择对应选项 id；若解析字母与选项文字冲突，以答案文字匹配到的选项为准。\n"
        "答案/解析必须解释同一道题的核心关键词、人物、国家、制度或时期；如果解析明显在讲其他题、上一节/下一节、"
        "相邻题号或相邻栏目，即使题号相同也必须视为 answer_conflict 跳过，禁止硬配。\n"
        "输出前逐题自检：correct_option_id 指向的选项文字必须与答案/解析一致，否则整题跳过。\n"
        "如果候选显示是简答题、论述题或其他主观题，且答案候选里能整理出参考答案，则输出 short_answer，格式为 "
        "{\"question_type\":\"short_answer\",\"stem\":\"...\",\"reference_answer\":\"...\",\"analysis\":\"...\"}。\n"
        "每题必须输出 analysis，优先使用答案来源的解析；如果解析很短，也要保留答案依据，禁止省略。\n"
        "如果候选信息不足以稳定映射到当前支持的题型，就整题跳过，不要硬凑成 multiple_choice。\n"
        "跳过任何 question_candidate 时，必须在顶层 skipped_reasons 数组说明 section、number、stem 摘要和 reason，"
        "reason 只能是 out_of_scope、missing_answer_candidate、unsupported_question_type、answer_conflict 或 insufficient_source。\n"
        "最终 questions 数量应等于范围内可配对 question_candidates 数量；输出前按候选顺序逐题核对是否漏掉模拟练习和论述题。\n"
...
def build_palace_quiz_pdf_review_prompt(extra_prompt: str) -> str:
    normalized_extra_prompt = str(extra_prompt or "").strip()
    return (
        "你是题库最终范围审核助手，只输出最终题库 JSON：{\"questions\":[...]}。\n"
        "任务：检查输入 questions，只保留满足用户补充要求的题目。\n"
        "如果用户补充要求限定了范围、题型、关键词或页码关注点，只保留明确满足要求的题；"
        "如果没有给出明确范围限制，则保持原题不删。\n"
...
def build_palace_quiz_generation_user_text(*, source_label: str, is_pdf_question_answer_pairing: bool) -> str:
