...
def ensure_session_progress_schema() -> None:
    with engine.begin() as conn:
        ...
        if "session_progress" not in existing_tables:
            conn.exec_driver_sql(
                ...
                CREATE TABLE session_progress (
                    ...
                    mini_palace_review_schedule_id INTEGER NULL,
            ...
            conn.exec_driver_sql(
                ...
                "WHERE session_kind = 'mini_review' AND mini_palace_review_schedule_id IS NOT NULL"
            )
            conn.exec_driver_sql(
        ...
        if "mini_palace_id" not in existing_columns:
            conn.exec_driver_sql(
                "ALTER TABLE session_progress ADD COLUMN mini_palace_id INTEGER"
            )
        if "mini_palace_review_schedule_id" not in existing_columns:
            conn.exec_driver_sql(
