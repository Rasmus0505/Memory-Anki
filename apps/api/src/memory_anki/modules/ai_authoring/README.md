...
Owns: AI runs, batch generation, imports, drafts, quality checks, publish plans.

Does not own: published content, questions, preferences.
...
Allowed business-module dependencies: library, memory_content, assessment, preferences. Imports must target the dependency's `public.py` or `contract` package.
