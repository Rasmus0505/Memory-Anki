...
def list_session_events(session: Session, study_session_id: str) -> list[dict[str, Any]]:
    rows = (
        ...
        .filter_by(study_session_id=study_session_id)
...
def _status_for(
    first_events: list[MindMapRecallEvent], retry_by_session: dict[str, MindMapRecallEvent]
) -> tuple[str, str, int]:
...
def list_node_mastery(
    session: Session, palace_id: int, *, weak_only: bool = False
) -> list[dict[str, Any]]:
    rows = (
        session.query(MindMapRecallEvent)
        .filter_by(palace_id=palace_id, source_scene="formal_review")
        .order_by(MindMapRecallEvent.occurred_at, MindMapRecallEvent.created_at)
        .all()
    )
    effective = _effective_events(rows)
    labels = {
        row.node_uid: row.label
        for row in session.query(MindMapNodeLabel).filter_by(palace_id=palace_id).all()
    }
...
def set_node_label(
    session: Session, palace_id: int, node_uid: str, label: str | None
) -> dict[str, Any]:
