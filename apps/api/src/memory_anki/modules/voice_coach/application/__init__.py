...
def _resolve_legacy_tts_runtime(
    ...
    *,
    ...
    runtime = resolve_scenario_runtime(session, "tts_voice_coach", ai_options=ai_options)
    if not is_dashscope_compatible_provider(runtime.provider):
        return runtime
    api_key = (
        runtime.api_key
        if has_non_empty_config(session, "dashscope_api_key")
        else str(DASHSCOPE_API_KEY or "").strip()
    )
    base_url = (
        runtime.base_url
        if has_non_empty_config(session, "dashscope_base_url")
...
def synthesize_voice_coach_event(
    ...
    *,
    ...
    resolved_runtime = _resolve_legacy_tts_runtime(session, ai_options=ai_options)
