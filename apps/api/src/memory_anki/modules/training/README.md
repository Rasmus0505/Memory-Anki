...
Owns: freestyle, recall, review, reading and practice orchestration.

Does not own: content persistence, question ownership, progress storage.
...
Allowed business-module dependencies: memory_content, assessment, learning_record, preferences. Imports must target the dependency's `public.py` or `contract` package.
