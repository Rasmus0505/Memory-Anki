...
describe("MindMapReviewFlow modes", () => {
  ...
  it("shows a mobile guided review rail without moving the viewport during card navigation", async () => {
    ...
    await waitFor(() => {
      ...
      expect(getLatestMindMapEditorSurfaceProps()?.focusRequestNodeUid).toBeUndefined();
      expect(getLatestMindMapEditorSurfaceProps()?.focusRequestNonce).toBeUndefined();
    });
  ...
  it("uses Space to advance the currently selected review node without firing inside inputs", async () => {
    ...
    await act(async () => {
      getLatestMindMapEditorSurfaceProps()?.onNodeActive?.([{ uid: "root", text: "Root" }]);
    });
    await act(async () => {
  ...
  it("keeps readonly right-click branch handling wired through the frame", async () => {
    ...
    await act(async () => {
      getLatestMindMapEditorSurfaceProps()?.onNodeContextMenu?.([{ uid: "child", text: "Child" }]);
    });
  ...
  it("lets root right-click hide revealed descendants while keeping the root visible", async () => {
    ...
    await act(async () => {
      getLatestMindMapEditorSurfaceProps()?.onNodeContextMenu?.([{ uid: "root", text: "Root" }]);
    });
