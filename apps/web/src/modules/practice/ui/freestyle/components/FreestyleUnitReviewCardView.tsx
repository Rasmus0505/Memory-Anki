...
export function FreestyleUnitReviewCardView({
  ...
  return (
    <section className="flex h-full min-h-0 flex-col gap-2.5 sm:gap-3" aria-label="永久标记复习单元">
      <header className="flex min-w-0 shrink-0 items-center gap-2 px-1">
        <span className="shrink-0 rounded-full border border-amber-300/25 bg-amber-300/10 px-2 py-1 text-[11px] font-semibold text-amber-100">
          永久标记
        </span>
        <div className="min-w-0 flex-1">
          ...
          {unit.title && card.palace_title && unit.title !== card.palace_title ? (
            <p className="truncate text-[11px] text-zinc-500 sm:text-xs">{card.palace_title}</p>
          ) : null}
        </div>
        {lastOperationId && !locked ? (
          <button
            type="button"
            disabled={busy}
            className="inline-flex h-9 shrink-0 items-center gap-1.5 rounded-full px-3 text-xs text-zinc-300 transition-colors hover:bg-white/10 hover:text-white disabled:opacity-40"
            onClick={() => void undo()}
          >
      ...
      <div className="min-h-0 flex-1 overflow-hidden rounded-[1.4rem] border border-white/10 bg-white shadow-[0_18px_50px_rgba(0,0,0,0.28)] sm:rounded-3xl">
      ...
      <footer className="shrink-0 rounded-[1.35rem] border border-white/10 bg-zinc-950/92 p-2 shadow-[0_12px_36px_rgba(0,0,0,0.38)] backdrop-blur-md sm:rounded-3xl sm:p-2.5">
        {selectedEffect ? (
          <div className="mb-2 rounded-xl border border-white/10 bg-white/[0.05] px-3 py-2 text-xs font-medium text-zinc-100 sm:text-sm">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          {ratings.map((item) => {
            const effect = currentEncounter.rating_effects.find((value) => value.rating === item.value)
            const hint = effect ? ratingEffectLabel(effect, retryAfterCards) : '计划不可用'
            ...
            return (
              <button
                ...
                className={cn(
                  'flex min-h-14 flex-col items-start justify-center rounded-2xl border px-3 py-2 text-left transition-colors active:scale-[0.98] disabled:pointer-events-none disabled:opacity-55',
                  item.className,
              ...
              >
                ...
                <span className="mt-0.5 text-[10px] leading-tight opacity-75 sm:text-[11px]">{hint}</span>
              </button>
            )
          })}
        </div>
