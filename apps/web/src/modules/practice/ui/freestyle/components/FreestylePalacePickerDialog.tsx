        ...
        <div className="space-y-1">
          {chapter.palaces.map((palace) => {
            ...
            return <label key={palace.id} className="flex min-h-9 items-center gap-2 rounded-lg px-2 py-1 text-sm hover:bg-muted/50" style={{ paddingLeft: `${32 + level * 18}px` }}>
              ...
              <span className="min-w-0 flex-1 truncate">{palace.resolved_title || palace.title}</span>
            </label>
          })}
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-4 sm:px-6">
        ...
        <div className="space-y-4">
          {subjects.map((subject) => <section key={subject.key} className="rounded-lg border border-border/70 bg-card/20 p-3">
            <h3 className="mb-2 text-sm font-semibold">{subject.title}</h3>
            {subject.chapters.map((chapter) => <ChapterRow key={chapter.key} chapter={chapter} selectedIds={draft} onToggle={toggle} level={0} />)}
            {subject.ungrouped ? <div className="mt-2 rounded-lg border-t border-border/50 pt-2"><div className="mb-1 text-sm font-medium">{subject.ungrouped.title}</div>{subject.ungrouped.palaces.map((palace) => <label key={palace.id} className="flex min-h-9 items-center gap-2 rounded-lg px-2 py-1 text-sm hover:bg-muted/50"><Checkbox checked={draft.includes(palace.id)} onCheckedChange={(checked) => toggle([palace.id], checked === true)} aria-label={`选择宫殿${palace.resolved_title || palace.title}`} /><span className="truncate">{palace.resolved_title || palace.title}</span></label>)}</div> : null}
          </section>)}
