    ...
    <Card className="border-border/70 bg-card/92">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-3"><Label>所属学科（可多选）</Label><Badge variant="secondary">已选 {selectedIds.length}</Badge></div>
          ...
          <div className="grid max-h-64 gap-2 overflow-y-auto sm:grid-cols-2">
            {loading ? <div className="text-sm text-muted-foreground">正在加载学科…</div> : filtered.map((subject) => {
              ...
              return <button key={subject.id} type="button" onClick={() => toggleSubject(subject.id)} className={`flex items-center gap-3 rounded-lg border px-3 py-3 text-left ${selected ? 'border-primary/50 bg-primary/10' : 'border-border/70 bg-background/60'}`}>
                <input type="checkbox" checked={selected} readOnly />
                <span className="size-3 rounded-full" style={{ backgroundColor: subject.color || '#6366f1' }} />
