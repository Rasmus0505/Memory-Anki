...
`library/PalaceListPage` and `library/PalaceLibraryPage` own shelf-to-freestyle entry composition; `modules/content` (palace-catalog UI) owns catalog behavior without importing practice internals. Shelf review enters `/freestyle?palaceId=<id>`; standalone `/review` pages are not registered.
