...
import { getEnglishReadingWorkspaceApi } from '@/features/english-reading/api'
import type { ReadingMaterial, ReadingWorkspaceResponse } from '@/shared/api/contracts'
import { EnglishContinueHero, EnglishStatStrip } from '@/features/english-shell'
  ...
  return (
    <div className="space-y-5" data-testid="english-hub-reading-tab">
      <EnglishContinueHero
        ...
        primaryHref={
          continueMaterial
            ? `/english-reading?material=${continueMaterial.id}`
            : '/english-reading'
        }
        empty={false}
        secondary={
...
function MaterialRow({ item }: { item: ReadingMaterial }) {
  return (
    <Link
      to={`/english-reading?material=${item.id}`}
      className={cn(
