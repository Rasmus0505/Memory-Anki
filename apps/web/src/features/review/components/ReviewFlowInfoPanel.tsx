import { PenLine } from 'lucide-react'
import { formatDuration } from '@/entities/session/model'

interface ReviewFlowInfoPanelProps {
  visibleNonRootCount: number
  revealedNonRootCount: number
  redNodeCount: number
  effectiveSeconds: number
  persistProgress: boolean
  timer: {
    effectiveSeconds: number
    idleSeconds: number
    pauseCount: number
    status: 'idle' | 'running' | 'paused' | 'completed'
    start: (meta?: Record<string, boolean | number | string | null>) => void
    pause: (meta?: Record<string, boolean | number | string | null>) => void
    resume: (meta?: Record<string, boolean | number | string | null>) => void
    adjustDuration: (seconds: number) => void
  }
  fullscreen: boolean
}

export function ReviewFlowInfoPanel({
  visibleNonRootCount,
  revealedNonRootCount,
  redNodeCount,
  effectiveSeconds,
  persistProgress,
  timer,
  fullscreen,
}: ReviewFlowInfoPanelProps) {
  return (
    <>
      <div className="rounded-lg border border-border/70 bg-card p-4">
        <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
          <PenLine className="size-4" />
          当前状态
        </div>
        <div className="mt-3 space-y-3 text-sm text-muted-foreground">
          <div className="rounded-lg border border-border/70 bg-background/70 px-3 py-3">
            已出现 {visibleNonRootCount} 个知识点，已揭示 {revealedNonRootCount} 个知识点，
            红标 {redNodeCount} 个知识点，当前有效时长 {formatDuration(effectiveSeconds)}。
          </div>
          <div className="rounded-lg border border-dashed border-border/80 px-3 py-3">
            翻卡模式只接管鼠标揭示与标红，不影响导图正文。
          </div>
          {persistProgress ? (
            <div className="rounded-lg border border-dashed border-success/30 bg-success/5 px-3 py-3 text-success">
              未完成时会自动续练；完成或手动重开后会清空这次练习进度。
            </div>
          ) : (
            <div className="rounded-lg border border-dashed border-warning/30 bg-warning/5 px-3 py-3 text-warning">
              正式复习不会跨退出保留当前翻卡进度，但会记录本次有效时长。
            </div>
          )}
        </div>
      </div>
    </>
  )
}
