...
export function TimerAutomationSection({
  ...
  onShowFloatingTimerChange,
}: {
  draft: AutomationDraft
  onAutoStartChange: (checked: boolean) => void
  onKeepScreenAwakeChange: (checked: boolean) => void
  onShowFloatingTimerChange: (checked: boolean) => void
}) {
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-border/70 bg-card/70 p-4">
        <div>
          <div className="text-sm font-semibold text-foreground">悬浮计时器</div>
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            默认关闭，避免挡住学习内容。打开后可拖动、折叠，桌面端会显示独立悬浮窗。
          </p>
        </div>
        <label className="mt-4 flex items-start gap-3 rounded-lg border border-border/60 bg-background/60 px-3 py-3 text-sm">
          <input
            type="checkbox"
            className="mt-0.5 size-4"
            checked={draft.showFloatingTimer}
            onChange={(event) => onShowFloatingTimerChange(event.target.checked)}
          />
          <span>
            <span className="block font-medium text-foreground">显示悬浮计时器</span>
            <span className="mt-1 block text-xs text-muted-foreground">
              在屏幕上显示全局计时器。关闭后不再出现浮层或桌面悬浮窗。
            </span>
          </span>
        </label>
      </div>
