...
describe('ReviewCompletion FSRS receipt', () => {
  ...
  it('shows ratings, absolute next review, interval, and unrated nodes', async () => {
    render(
    ...
    expect(screen.getByText(/1天后|1天/)).toBeTruthy()
    expect(screen.getByText(/本次未评分 1 个节点保持到期/)).toBeTruthy()
    expect(screen.getByText(/仍到期 1 个节点/)).toBeTruthy()
  })
})
