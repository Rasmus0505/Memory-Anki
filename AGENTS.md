...
- This is a private two-device Windows product. Never hard-code one machine's paths. Runtime data is configured per machine in `local-config/memory-anki.local.json` (`local_app_home`) and points to that machine's Syncthing folder (for example `F:\memory anki data`; do not assume the drive letter). Syncthing performs cross-device synchronization outside the app; keep the app's `sync_enabled`, `sync_on_start`, and `sync_on_stop` false and leave `sync_root` null. Do not use Baidu `MemoryAnki-Sync`. Fallback: `MEMORY_ANKI_HOME` / `%LOCALAPPDATA%\MemoryAnki`.
