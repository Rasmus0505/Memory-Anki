...
- This is a private two-device Windows product. Never hard-code one machine's paths. Runtime data is configured per machine in `local-config/memory-anki.local.json` (`local_app_home`); prefer `vol:MemoryAnki/memory anki data` for the shared USB stick, or `MEMORY_ANKI_HOME` / `%LOCALAPPDATA%\MemoryAnki` as fallback.
