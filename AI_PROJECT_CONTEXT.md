...
Memory Anki 是一个自用的本地桌面学习产品，核心是“记忆宫殿 + 间隔重复 + AI 辅助知识生产”。

它不是公开 SaaS，不考虑移动端，不面向多用户，也不需要把数据上传到云服务。主要运行在两台 Windows 电脑上，运行时数据通过百度网盘同步空间跨设备同步。因此代码修改时最重要的隐含约束是：**不能写出只能在当前电脑运行的路径、配置或数据假设**。
...
- 开发模式：后端和 Vite 前端分进程，Vite 将 `/api` 代理到后端
- 生产/本地启动：后端可以同时服务 API 和已构建前端
