...
- 开发模式：后端和 Vite 前端分进程，Vite 将 `/api` 代理到后端；PWA 默认页面为 `/freestyle`
- 生产/本地启动：后端可以同时服务 API 和已构建前端，PWA 脚本统一使用 `127.0.0.1:8012`
- PWA 私有访问：Tailscale Serve 将 HTTPS 转发到 `http://127.0.0.1:8012`，手机访问 `https://desktop-lp-2026481850.tail92e457.ts.net/freestyle`
...
- 手机安装/访问：`https://desktop-lp-2026481850.tail92e457.ts.net/freestyle`
- 停止服务：`.\stop-pwa.bat`
