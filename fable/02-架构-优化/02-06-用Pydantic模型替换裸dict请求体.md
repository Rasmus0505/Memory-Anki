...
| 2026-07-09 | Codex | 完成实现与验证 | 新增 sessions/freestyle/knowledge domain Pydantic 请求模型，关键写接口改为 `model_dump(exclude_unset=True, exclude_none=False)` 后传服务层；保留 `POST /study-sessions/from-time-record` 与 `PUT /subjects/{id}/editor` 为自由结构 `dict`，代码注释说明 legacy timer mixed-case payload 与 mind-map editor open-ended state；`python -m pytest tests/test_study_session_routes.py tests/test_freestyle_routes.py tests/test_knowledge_routes.py tests/test_palace_chapter_binding.py -q` 通过（31 passed, 5 warnings），`python -m ruff check src/memory_anki/modules/sessions src/memory_anki/modules/freestyle src/memory_anki/modules/knowledge` 通过 |
